...
static func get_water_body_meshes(high_level_terrain: HighlevelTerrain, debug_color_dict: DebugColorDict) -> Array:  # -> Array(Mesh)
	var meshes: Array = []
	for lake in high_level_terrain.get_lakes():
		meshes.append(_get_water_body_mesh(lake, debug_color_dict))
	return meshes

static func _get_water_body_mesh(lake: Region, debug_color_dict: DebugColorDict) -> Mesh:
	var surface_tool: SurfaceTool = SurfaceTool.new()
